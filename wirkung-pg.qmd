...
Auf dieser Seite seht ihr alle Antworten zu Fragen, die die Programmgruppe (PG) betreffen - also euer Feedback an uns! 

## Informationskanäle bei TEN SING
  ...
  info_komplett_plot <- ggplot(
    ...
    ggiraph::geom_col_interactive(
      aes(
        ...
        data_id = paste0("left_", info_komplett)
      )
    ) +
    coord_polar(theta = "y") +
    scale_fill_manual(
  ...
  info_aenderung_plot <- ggplot(
    ...
    ggiraph::geom_col_interactive(
      aes(
        ...
        data_id = paste0("right_", info_wege_aenderung)
      )
    ) +
    coord_polar(theta = "y") +
    scale_fill_manual(
...
prepare_info_channel_data <- function(y, data, plot_title, levels, y_limits = NULL, y_breaks = NULL) {
  agg <- data %>%
    ...
    dplyr::mutate(perc = 100 * n / length(unique(data$tn_id)),
                  n_tensingende = length(unique(data$tn_id)))
...
render_info_channel_plot <- function(channel_data) {
  ...
  p <- ggplot(agg, aes(x = info_weg, y = perc, fill = info_weg)) +
    ggiraph::geom_col_interactive(
      aes(
        tooltip = paste0(fmt_perc(perc/100), " (", n, " von ", n_tensingende, "):\n",info_weg),
        data_id = info_weg
      )
    ) +
...
render_gewuenschte_info_channel_plot <- function(channel_data) {
  agg <- channel_data$agg

  p <- ggplot(agg, aes(x = info_weg, y = n, fill = info_weg)) +
    ggiraph::geom_col_interactive(
      aes(
        tooltip = sprintf("%s: %i", info_weg, n),
        data_id = info_weg
      )
    ) +
    scale_x_discrete(labels = function(x) str_wrap(x, width = 12)) +
...
render_year_tabset(
  ...
  render_fn = render_gewuenschte_info_channel_plot
)
...
render_participation_comparison <- function(data) {
  ...
  p <- ggplot(combined, aes(y = type_veranst, x = perc, fill = teilnahme, group = forcats::fct_rev(teilnahme))) +
    ggiraph::geom_col_interactive(aes(tooltip = sprintf("Teilnahme %s: %s\n%s aller Aktiven", type_veranst, teilnahme, fmt_perc(perc))), position = "dodge") +
    ...
    scale_fill_manual(values = colors) + # alternatively to legend label: , labels = c("teilgenommen", "nicht teilgenommen", "keine Angabe")
    theme(
      legend.position = "bottom",
      plot.title = element_text(hjust = 0.5, family = TS_FONT_FAMILY)
    )
...
render_participation_barriers <- function(data) {
  ...
  p <- ggplot(barriers, aes(y = grund, x = percent)) +
    geom_col_interactive(aes(tooltip = fmt_perc(percent)), fill = TS_GREEN) +
...
```{r}
westbund_perc_yes <- participation_curr$dtl_region_summary |> filter(region_grouped == "Westbund") |> pull(pct_ja)
```

Spannend ist dabei zu sehen, ob es bei der Teilnahme an deutschlandweiten Veranstaltungen regionale Unterschiede gibt. Dafür haben wir pro Region visualisiert, welcher Anteil der Menschen, die die Umfrage ausgefüllt haben, bereits an einer deutschlandweiten Veranstaltung teilgenommen hat. Am Beispiel des Westbunds heißt das, dass `r round(westbund_perc_yes, 2)`% der Menschen, die die Umfrage ausgefüllt haben und sich dem Westbund zugehörig fühlen, auch mal eine deutschlandweite Veranstaltung besucht haben.
Gekennzeichnet mit „deutschlandweit“ seht ihr wie viel Prozent der Umfrageteilnehmenden über alle Umfragen hinweg bereits an einem deutschlandweiten Angebot teilgenommen haben.
...
Wie sieht es bei den regionalen Veranstaltungen aus? Gibt es Unterschiede zwischen den Regionen? Auch hier haben wir die Antworten für jede der TEN SING Regionen visualisiert.
...
Gehen TEN SINGer*innen lieber zu deutschlandweiten oder regionalen Veranstaltungen? Hier seht ihr den Vergleich.
...
render_unterstuetzung_plot <- function(data) {
  ...
  p_bar <- ggplot(
      ...
      aes(
        ...
        y = bedarf,
        x = prop,
        tooltip = paste0(bedarf, "\n", bedarf_value, ": ", fmt_perc(prop), " der Ortsgruppen")
      )
    ) +
...
Vielen Dank, dass du Teil dieser Befragung warst oder dich mit den Ergebnissen auseinandergesetzt hast. Wenn du Fragen hast, kannst du dich jederzeit an [tensing@cvjm.de](mailto:tensing@cvjm.de) wenden. Bitte hilf uns, die Befragung auch in den kommenden Jahren weiterhin durchführen zu können und Teile den aktuellen Link zur Befragung oder gib uns Feedback, was wir besser machen können.
