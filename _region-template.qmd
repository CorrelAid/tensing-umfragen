...
logo_path <- file.path("..", "logos", paste0(params$region, ".png"))
...
Im Folgenden seht ihr eine Übersicht über die einzelnen Ortsgruppen der Region `{r} params$region`.
Laut Teilnehmer*innen-Fragebogen gibt es `{r} og_nach_tn` Ortsgruppe(n).<sup>1</sup> Davon haben wir `{r} og_nach_og` über den Ortsgruppen Fragebogen erreicht.<sup>2</sup> 
...
create_region_stats_table_westbund <- function(region_overview) {
  ...
  reactable::reactable(
    ...
    columns = list(
      ...
      n_og = reactable::colDef(name = "Ortsgruppen", footer = function(values) total_og, minWidth = 110, maxWidth = 120),
...
angebot_anz_theater <- curr$tn$long$angebote_vor_ort %>%
  ...
  pull(tn_id) %>%
  n_distinct()
angebot_perc_theater=round(100*angebot_anz_theater/angebot_anz_ges,2)
```

:::{.callout-tip collapse="false"}

## Interpretationshilfe
Insgesamt `{r} angebot_anz_ges` Personen haben im Jahr `{r} current_year` eine Antwort auf diese Frage gegeben. `{r} angebot_anz_theater` Personen gaben wiederum an, dass sie gerne am Theaterworkshop/Theaterangebot teilnehmen. `{r} angebot_anz_theater` von `{r} angebot_anz_ges` Personen entspricht `{r} angebot_perc_theater`% Prozent. Diesen Wert seht ihr im oberen Schaubild visualisiert.
...
`r if (!zugang_has_subregions) sprintf("**Beispielinterpretation**: %.1f%% der Teilnehmenden gaben im Jahr %s **als einen** ihrer Zugangswege zu TEN SING an, von einer anderen Person mitgenommen worden zu sein.", round(example_proz_curr,1), current_year) else ""`
...
plot_massnahmen <- function(mass_count_agg) {
  ...
  p <- ggplot(mass_count_agg, aes(y = label_short, x = n, tooltip = massnahme)) +
    ggiraph::geom_col_interactive(fill = TS_GREEN) +
    scale_y_discrete(limits = rev) +  
...
Hier seht ihr die Antworten der Ortsgruppen. Wenn Vorjahresdaten vorhanden sind, werden die Jahre `{r} current_year` und `{r} previous_year` gegenübergestellt. 
...
add_explanation_to_bedarf <- function(og_ueb_sum){
  ...
  mutate(
    erklärung_bedarf = case_when(
      bedarf_label == "hoch"   ~ "Der Bedarf ist hoch und",
