...
plot_agg_angebot <- function(angebote_agg) {
  ...
  region_subtitle <- if (length(reg) == 1) reg else NULL;
...
render_ihilfe <- function (data) {
  cat("::: {.callout-tip}\n\n")
  print(data)
  s <- "Insgesamt `{r} angebot_anz_ges` Personen haben im Jahr `{r} current_year` eine Antwort auf diese Frage gegeben. `{r} angebot_anz_theater` Personen gaben an, dass sie gerne am Theaterworkshop/Theaterangebot teilnehmen. `{r} angebot_anz_theater` von `{r} angebot_anz_ges` Personen entspricht `{r} angebot_perc_theater`% Prozent. Diesen Wert seht ihr im oberen Schaubild visualisiert. Da eine Mehrfachauswahl möglich war, summieren sich die Werte auf mehr als 100%."
  cat("## Hallo\n\n")
  cat(s, sep = "\n")
  cat(":::\n")
}
