...
# WORTER
TN_DATA$long$woerter <- pivot_cols_long(tn_df, tn_cfg$QS_WORT$col_name, "wort") %>% select(tn_id, wort)

# EIGENSCHAFTEN -----------
eig_long <- pivot_cols_long(tn_df, tn_cfg$QS_EIGENSCHAFTEN$col_name, values_to = "value")
eig_long <- eig_long %>% 
    left_join(tn_cfg$QS_EIGENSCHAFTEN %>% select(label, name, col_name), by = "col_name") %>% 
...
TN_DATA$wide$aussagen <- tn_df %>% select(tn_id, tn_cfg$CN_AUSSAGEN_LIKERT) %>% 
  select(tn_id, sort(tidyselect::peek_vars())) %>%   
  rename_with(~tn_cfg$QS_AUSSAGEN_LIKERT$name, all_of(starts_with("group")))


# ANGEBOTE VOR ORT ---------------
angebote_m <- get_mapping(tn_choices, tn_cfg$Q_ANGEBOTE_VOR_ORT$select_from_list_name)
...
TN_DATA$data <- TN_DATA$data %>%
    dplyr::mutate(og_name_orig = if_else(!is.na(og_name_orig), og_name_orig, "")) %>%
    mutate(og_name_cleaned = str_remove_all(og_name_orig, stringr::regex("tensing|ten sing|ten-sing", ignore_case = TRUE)) %>% stringr::str_trim())
...
m <- get_mapping(tn_choices, tn_cfg$Q_VERANTWORTUNG_JANEIN$select_from_list_name)
...
hilfe_sonst <- tn_df %>%
    ...
    select(tn_id, hilfsangebot_sonst = !!tn_cfg$CN_VERANTWORTUNG_SUPPORT_SONST) %>% 
    mutate(hilfsangebot = "sonstiges")

# all other ones 
m_hilfe <- get_mapping(tn_choices, tn_cfg$Q_VERANTWORTUNG_SUPPORT$select_from_list_name)
...
TN_DATA$data$wahrsch_empfehlung <- tn_df %>% 
    pull(!!tn_cfg$CN_EMPFEHLUNG) %>% as.numeric() # we use the numeric values 
