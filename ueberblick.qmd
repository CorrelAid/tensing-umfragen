...
reactable(
  og_region_df,
  groupBy = "og_region",
  defaultSorted = list(og_name = "desc"),
  columns = list(
    ...
    slug = colDef(name = "Details", html = TRUE, cell = function(value) { 
      return("")
    }, aggregate = JS("
    function(values, rows) {
      if (!values || values.length === 0 || !values[0]) return '';
      const slug = values[0];
      return '<a href=\"/region/' + slug + '.html\" rel=\"noopener noreferrer\" style=\"text-decoration: none;\">↗️</a>';
    }
  "))
  )
)
...
Aussage Gute Entscheidung TEN SING 
