...
if (tn_nach_og >= tn_nach_tn) {
   s <- sprintf("Laut Schätzung der Ortsgruppen-Leitenden sind bei TEN SING ca. <b>%i Teilnehmer\\*innen</b> aktiv.<sup>1</sup> <br><br>%i haben wir davon über den Teilnehmer\\*innen Fragebogen erreicht.<sup>2</sup>", tn_nach_og, tn_nach_tn)
} else {
  s <- sprintf("<b>%i TEN SINGende</b> haben den Teilnehmer*innen-Fragebogen ausgefüllt.<sup>2</sup> <br><br>Laut Schätzung der Ortsgruppen-Leitenden sind bei TEN SING aber nur ca. <b>%i Teilnehmer\\*innen</b> aktiv. Diese Differenz kann daran liegen, dass (noch) nicht genügend Ortsgruppenleitungen den Ortsgruppen-Fragebogen ausgefüllt haben.<sup>1</sup>", tn_nach_tn, tn_nach_og)
}
...
Insgesamt engagieren sich über die Ortsgruppen verteilt aktuell also **`{r} insgesamt_og_curr`  Personen aktiv bei TEN SING.**<sup>4</sup>

```{r}
#| results: asis
if (og_nach_tn >= og_nach_og) {
   s <- sprintf("
  Deutschlandweit gibt es laut der Angaben von Teilnehmer*innen aktuell <b>%i Ortsgruppen</b> bei TEN SING.<sup>5</sup> Davon haben wir %i über den Ortsgruppen-Fragebogen erreicht.<sup>6</sup>", og_nach_tn, og_nach_og)
} else {
  s <- sprintf("
  <b>%i Ortsgruppen</b> haben den Ortsgruppen-Fragebogen ausgefüllt.<sup>6</sup> Aus %i Ortsgruppen haben TEN SINGende den Teilnehmer*innen-Fragebogen beantwortet.<sup>5</sup>", params$region, og_nach_og, og_nach_tn)
}
...
<sup>2</sup> Dies ist die Gesamtanzahl ausgefüllter Umfagen des `{r} fmt_fragebogen("tn", url=curr$cfg$tn_cfg$URL)`
...
<sup>6</sup> Dies ist die Gesamtanzahl ausgefüllter Umfragen des `{r} fmt_fragebogen("og", url=curr$cfg$og_cfg$URL)`s
...
Im Folgenden seht ihr eine Übersicht über die einzelnen TEN SING Regionen. Aufgelistet sind die Ortsgruppen jeder Region sowie die Anzahl der aktiven TEN SINGer*innen pro Ortsgruppe und aggregiert pro Region.
...
Mit einem Klick auf den ▶ kannst du jede Region öffnen und eine Liste ihrer Ortsgruppen sehen.
...
Im Folgenden seht ihr eine Übersicht über die Geschlechter- und Altersverteilung der Menschen bei TEN SING. 
...
::: {.callout-note collapse="true"}
## Info
Um die Privatsphäre aller Personen, die an der Umfrage teilgenommen haben, zu schützen, wird eine Übersicht über die Geschlechter- und Altersverteilung der Menschen bei TEN SING nur in aggregierter Form angezeigt. 
:::
...
Im Durchschnitt haben die TEN SING Mitarbeitenden ein Stundenpensum von `{r} ma_avg_stunden_curr` Stunden pro Woche.<sup>1</sup> Über alle Ortsgruppen zusammen arbeiten Mitarbeitende `{r} ma_total_stunden_curr` Stunden für TEN SING pro Jahr.
...
**🎶 Pro Schuljahr finden ca. `{r} floor(avg_auftritte_curr)` bis `{r} ceiling(avg_auftritte_curr)` Auftritte statt.<sup>5</sup> 🎶**
...
Neben der engagierten Mitarbeit aller Beteiligten, macht insbesondere auch das Übernehmen von Verantwortung TEN SING erst möglich. Deshalb wollten wir von euch wissen, wie viele von euch aktiv Verantwortung bei TEN SING übernehmen. Was unterstützt Menschen dabei, Verantwortung zu übernehmen?

Im Jahr `{r} curr$year` gaben `{r} dplyr::filter(verantwortung_curr$verant_anteile, verantwortung_janein == "Ja")$perc` % der befragten TEN SINGer*innen an, Verantwortung zu übernehmen.  Welche Hilfsangebote sich TEN SINGende mit und ohne Verantwortung wünschen, siehst du in der folgenden Visualisierung.
...
render_verantwortung_plot <- function(data, y_limit = verantwortung_y_limit) {
  summary_tbl_pct <- data$summary_tbl |>
    ...
    dplyr::mutate(
      ...
      tooltip = paste0(
        ...
        "Anteil: ", scales::percent(pct, accuracy = 1)
      ),
  ...
  p <- ggplot(
      ...
      labs(
        title = stringr::str_wrap("Welche Hilfsangebote wünschen sich Personen mit und ohne Verantwortung?", width = 80),
        x = "Hilfsangebot",
        y = "Anteil",
        fill = NULL
      ) +
...
Das Übernehmen von Verantwortung wird unterschiedlich wahrgenommen je nachdem, ob eine Person aktiv Verantwortung übernimmt oder nicht. Im Jahr `{r} curr$year` wünschen sich TEN SINGende mit Verantwortung vor allem *"`{r} verantwortung_curr$max_hilfsangebote$ja`"* als Unterstützung. TEN SINGende ohne Verantwortung sagen, *"`{r} verantwortung_curr$max_hilfsangebote$nein`"* würde am meisten helfen, um Verantwortung zu übernehmen.
...
plot_wahrsch_empfehlung <- function(y, y_limit = wahrsch_empf_y_limit){
  ...
  p <- ggplot(df, aes(x = factor(value), y = percent, fill = factor(value))) +
    ...
    labs(
      ...
      title = "Wie wahrscheinlich würdest du TEN SING weiterempfehlen?"
    ) +
    guides(fill = "none") +
    theme(
      strip.text = element_text(face = "bold", size = 12),
...
- ... [Thüringen](./region/thueringen.qmd)
- ... [Süden](./region/sueden.qmd)
