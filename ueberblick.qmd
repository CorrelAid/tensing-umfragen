...
Laut Schätzung der Ortsgruppen-Leitenden sind bei TEN SING ca. **`{r} tn_nach_og` Teilnehmer\*innen** aktiv.<sup>1</sup> 

`{r} tn_nach_tn` haben wir davon über den Teilnehmer\*innen Fragebogen erreicht.<sup>2</sup> 
...
Deutschlandweit gibt es laut der Angaben von Teilnehmer*innen aktuell **`{r} og_nach_tn` Ortsgruppen** bei TEN SING. <sup>5</sup> Davon haben wir `{r} og_nach_og` über den Ortsgruppen Fragebogen erreicht.<sup>6</sup>
...
<sup>3</sup> Die entsprechenden Fragen im `{r} fmt_fragebogen("og", url=curr$cfg$og_cfg$URL)` lautet: 
...
<sup>6</sup> Dies ist die Gesamtanzahl ausgefüllter Umfagen des `{r} fmt_fragebogen("og", url=curr$cfg$og_cfg$URL)`
...
TEN SINGer*innen treffen sich am liebsten am `{r} fav_day_curr` in ihren Ortsgruppen.<sup>3</sup> Im Schnitt dauert ein Treffen `{r} avg_dur_curr` Minuten.<sup>4</sup> Auf ein Jahr hochgerechnet sind das `{r} avg_yearly_hours_curr` Stunden die sich jede Ortsgruppe im Durschnitt trifft. 
...
render_verantwortung_plot <- function(data, y_limit = verantwortung_y_limit) {
  ...
  p <- ggplot(
      ...
      scale_fill_manual(values = pal_yes_no) +
      scale_y_continuous(
...
du willst mehr zu deiner Region erfahren...?!
