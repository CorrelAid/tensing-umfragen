...
**Hinweis**: Die Datenquellen werden euch gebündelt am Ende der Seite angezeigt.
:::
...
Die folgenden Visualisierungen gehen Fragen rund um das Gefühl der Gemeinschaft nach. Wie geschützt und wertschätzend geht es bei TEN SING zu? Entsteht für euch das Gefühl eines Safe Space?

Zunächst wollten wir dafür von euch wissen, ob ihr bei TEN SING so sein könnt, wie ihr das gerne möchtet.
...
Ist TEN SING ein geschützter Raum für euch?
...
Wie sehr achten die TEN SINGenden aufeinander?
...
Auch wollten wir von euch wissen, ob bei TEN SING ein Gefühl von Gemeinschaft herrscht.
...
In diesem Teil gehen wir der Frage nach, inwiefern Menschen, die sich bei TEN SING engagieren, durch die Arbeit an Selbstvertrauen und Selbstwirksamkeit dazugewinnen.

Dafür wollten wir von euch wissen, ob ihr euch durch euer Engagement bei TEN SING selbstbewusster fühlt. So habt ihr geantwortet:
...
Auch hat uns interessiert, ob ihr euch durch die Arbeit bei TEN SING Dinge zutraut, die ihr euch davor nicht getraut habt. Die Abbildung zeigt eure Antworten:
...
Die Arbeit bei TEN SING ist gemeinschaftlich. Und dennoch ist die Entwicklung jeder einzelnen Person wichtig. In diesem Abschnitt beleuchten wir deshalb eure persönliche Entwicklung.

Wie sehr habt ihr das Gefühl, euch weiterentwickelt zu haben, seitdem ihr bei TEN SING aktiv seid?
...
Habt ihr das Gefühl, euch bei TEN SING ausprobieren zu können?
...
Und zu guter Letzt wollten wir wissen, wie gut ihr Dinge, die ihr bei TEN SING erlernt, auch außerhalb einsetzen könnt.
...
Lernt ihr bei TEN SING neue Dinge?
...
In der Mehrzahl setzen sich Aktive bei TEN SING mit ihrem Glauben auseinander. `{r} round(glaube_rolle_tabs$curr$share_eher_yes * 100, 2)`% der Aktiven bewerten die Aussage "*`{r} glaube_rolle_tabs$curr$question`*" als eher bis voll und ganz zutreffend.
...
render_offenheit_plot <- function(tab_data) {
  ...
  p <- ggplot(
      ...
      scale_y_continuous(labels = fmt_perc, limits = c(0, 0.6)) +
      labs(
        y = "% der Aktiven",
        x = "Bewertung",
        title = stringr::str_wrap(tab_data$question, 60),
