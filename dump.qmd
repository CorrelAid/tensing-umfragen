...
<!--
OLD TEXT ELEMENTS AND SOME PLOTS THAT ARE POTENTIALLY NOT USED? OR MAYBE THEY ARE?
...
ggplot2::theme_set(theme_tensing_base) # defined in viz_config
...
ggplot(og_data, aes(x = anzahl_ma_leitung))+
  ...
  geom_text(x = mean(og_data$anzahl_ma_leitung, na.rm = TRUE),  nudge_x = -0.1, y = 5, label = paste("Mittelwert =", mean(og_data$anzahl_ma_leitung, na.rm = TRUE) %>% round(2)))+
    labs(x = "Anzahl Mitarbeitende/Leitung", y = "Anzahl Gruppen", title = "Anzahl der Mitarbeitenden")
```
:::
